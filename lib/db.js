...
function buildSeed() {
  ...
  return {
    ...
    employees: [
      { id: 'e1', name: 'أحمد سالم',  nationalId: '1012345678', phone: '0501234567', department: 'العمليات',     jobTitle: 'سائق',          email: 'ahmed@fna.sa',  status: 'active', vehicleId: 'v1', ...(IS_PROD ? {} : { quickPin: '1357' }), createdAt: now },
      { id: 'e2', name: 'سارة علي',   nationalId: '1098765432', phone: '0509876543', department: 'اللوجستيات',   jobTitle: 'سائقة',         email: 'sara@fna.sa',   status: 'active', vehicleId: 'v2', ...(IS_PROD ? {} : { quickPin: '2468' }), createdAt: now },
      { id: 'e3', name: 'خالد حسن',  nationalId: '1056781234', phone: '0505551234', department: 'الصيانة',      jobTitle: 'فني',           email: 'khalid@fna.sa', status: 'active', vehicleId: 'v3', ...(IS_PROD ? {} : { quickPin: '1597' }), createdAt: now },
      { id: 'e4', name: 'منى فهد',   nationalId: '1023344556', phone: '0502223344', department: 'العمليات',     jobTitle: 'سائقة',         email: 'mona@fna.sa',   status: 'active', vehicleId: 'v4', ...(IS_PROD ? {} : { quickPin: '2684' }), createdAt: now },
      { id: 'e5', name: 'علي ناصر',  nationalId: '1077889900', phone: '0507778899', department: 'الإدارة',      jobTitle: 'مشرف عمليات',  email: 'ali@fna.sa',    status: 'active', vehicleId: 'v5', ...(IS_PROD ? {} : { quickPin: '3141' }), createdAt: now },
      { id: 'e6', name: 'هند راشد',  nationalId: '1033445566', phone: '0503334455', department: 'اللوجستيات',   jobTitle: 'سائقة',         email: 'hind@fna.sa',   status: 'active', vehicleId: 'v6', ...(IS_PROD ? {} : { quickPin: '2718' }), createdAt: now },
    ],
    alerts: [
    ...
    gpsHistory: {},
    cities: [
      { id: 'c1', name: 'الرياض', createdAt: now },
      { id: 'c2', name: 'جدة', createdAt: now },
      { id: 'c3', name: 'الدمام', createdAt: now },
    ],
    projects: [
      { id: 'p1', name: 'مشروع المنطقة الوسطى', cityId: 'c1', createdAt: now },
      { id: 'p2', name: 'مشروع المنطقة الغربية', cityId: 'c2', createdAt: now },
      { id: 'p3', name: 'مشروع المنطقة الشرقية', cityId: 'c3', createdAt: now },
    ],
    maintenanceCards: [],
    approvedForms: [],
    quickAccessUsers: [],
...
function readStore() {
  ...
  ensureDir();
  if (fs.existsSync(TMP_FILE) && !fs.existsSync(DATA_FILE)) {
    try { fs.renameSync(TMP_FILE, DATA_FILE); } catch { /* ignore stale tmp */ }
  } else if (fs.existsSync(TMP_FILE)) {
    try { fs.unlinkSync(TMP_FILE); } catch { /* ignore stale tmp */ }
  }
...
function _flush() {
  ...
  try {
    ...
    if (fs.existsSync(TMP_FILE)) {
      try { fs.unlinkSync(TMP_FILE); } catch { /* ignore tmp cleanup errors */ }
    }
